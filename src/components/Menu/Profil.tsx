...
const LanguageButton: React.FC<LanguageButtonProps> = ({ locale, value, handleLocale }) => {
	const { colors } = useTheme();
	return (
		<Block flex row align="center" gap={16} padding={"16px 32px"} border={`3px solid ${colors.secondary.main}`} radius={8} width={198}>
			<Input type="radio" value={value} name="lang" width={20} height={20} onChange={handleLocale} checked={locale === value} />
...
const Profil = () => {
	...
	const handleLocale = useCallback(
		async (e: React.ChangeEvent<HTMLInputElement>) => {
			...
			setLocale(newLocale);
			const newSettings = { ...settings, user: { ...settings.user, locale: newLocale } };
		...
		<>
			<Block flex column align="flex-start" justify="flex-start" gap={44} width={436}>
				<Block flex column align="flex-start" justify="flex-start" gap={16}>
					...
					<Button secondary row flex padding={"14px 32px"} gap={16} onClick={resetDevice}>
						<Logout />
						<Text p label>
							{t("common.logout")}
						</Text>
				...
				<Block flex column align="flex-start" height={192} justify="flex-start" gap={24} alignSelf="stretch">
					...
					{languages.map((lang) => (
						<LanguageButton key={lang} locale={locale} value={lang} handleLocale={handleLocale} />
					))}
				</Block>
			</Block>
		</>
