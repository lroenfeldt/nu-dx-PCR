...
const MenuItem: React.FC<MenuItemProps> = ({ IconComponent, label, iconProps = {}, onClick }) => {
	...
	return (
		<Block flex align="center" gap={8} radius={32} padding="16px 32px" onClick={onClick} cursor dropShadowSmall width={542}>
...
const Menu = ({ onClose }: { onClose: () => void }) => {
	...
	const menuItems = [
		...
		{ IconComponent: Update, label: "Update", onClick: () => setCurrentMenu(Menus.MAIN) },
	];
	...
	return (
		<Block height={"100%"}>
			<Block position="fixed" right={0} top={0} radius={16} white dropShadowLarge>
