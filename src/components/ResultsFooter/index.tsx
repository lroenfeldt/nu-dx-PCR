...
import { IResultsFooterProps } from "../../types/components";

const ResultsFooter: React.FC<IResultsFooterProps> = ({
  activeBarcode,
  testmethod,
  settings,
  barcodes,
  ...
  const currentBarcode = barcodes.find(
  ...
  const buttonText = `${resultPrefix} ${
    currentResult === "invalid"
      ...
      : testmethod.results.find(
          (result) =>
            result.name.includes(currentResult) || result.name === currentResult
        )?.["label" + locale.toUpperCase()] || currentResult
  }`;
          ...
          <Block>
            <Button
              style={{
                ...
                borderRadius: "4px",
                minWidth:
                  currentResult && currentResult.length > 9
                    ? 12 * currentResult.length
                    : undefined,
                transition: ".3s",
                position: "relative",
