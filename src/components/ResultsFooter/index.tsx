...
const ResultsFooter: React.FC<ResultsFooterProps> = ({ activeBarcode, testmethod }) => {
					...
					<Button
						...
						{barcodes.filter((barcode) => barcode.id == activeBarcode.id)[0]?.result == "invalid"
							...
							: testmethod.results.find(
							  ...
							  )?.["label" + locale?.toUpperCase()] || barcodes.filter((barcode) => barcode.id == activeBarcode.id)[0]?.result}

						<AlteredResult activeBarcode={barcodes.filter((barcode) => barcode.id == activeBarcode.id)[0]} testmethod={testmethod} />
						<Checkmark
