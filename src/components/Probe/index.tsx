...
const Probe: React.FC<ProbeProps> = ({ barcode, isActive, onClick, showResults = false, testmethod = null }) => {
		...
		return (
			<Block
				...
				style={{
					...
					backgroundColor: ["negative", "positive", "invalid"].includes(barcode.result)
				...
				<Text
					...
					fontWeight={700}
					lineHeight="normal"
					color={!(barcode.result == "") ? colors.secondary.main : colors.white.main}>
					{barcode?.label}
				</Text>
		...
		<Block
			...
			style={{
				...
				...((barcode.value === "TPC" || barcode.value === "NTC") &&
					settings.account.autoControl && { backgroundColor: colors.grey[600], cursor: "not-allowed" }),
				...(!barcode.checking && barcode.value.length >= 1 && !barcode.valid && { backgroundColor: colors.probe.invalid }),
			}}>
			<Text
				white
				fontSize="32px"
				...
				fontWeight={700}
				lineHeight="normal"
				color={!barcode.blocked && barcode.value.length >= 1 ? colors.secondary.main : colors.white.main}>
				{barcode?.label}
			</Text>
