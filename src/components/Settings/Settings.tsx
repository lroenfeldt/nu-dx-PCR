...
const Settings: React.FC<SettingsProps> = ({ visible }) => {
	const { setBarcodes, saveSettings, settings, setMenuOpen, clearSettings, updateAvailable, setUpdateAvailable } = useData();
		...
		<div>
			<ArrowBox direction={`top ${visible ? "active" : "disable"}`}>
				<div className="settingsContent">
					...
					{settings.isDev && <Switch label={settings.device.wellCount} onClick={handleWellCount} checked={settings.device.wellCount === 96} />}
