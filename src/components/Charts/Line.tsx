...
function Line({ barcode }: ILineProp) {
  ...
  for (let param in barcode.parameters) {
    ...
    let dataset = {
      label: parameters.find((p: { target: string }) => p.target == param.toUpperCase())?.label,
      data: parameter.curveData,
      borderColor: parameters.find((p: { target: string }) => p.target == param.toUpperCase())?.color,
      borderWidth: 3,
    };
    datasets.push(dataset);

    const iShowThreshhold = parameters.find((p: { target: string }) => p.target == param.toUpperCase())?.showThreshhold;
    if (iShowThreshhold) {
      datasets.push({
        label: parameters.find((p: { target: string }) => p.target == param.toUpperCase())?.label + ' Threshhold',
        data: new Array(40).fill(
          parameters.find((p: { target: string }) => p.target == param.toUpperCase())?.threshhold
        ),
        borderColor: parameters.find((p: { target: string }) => p.target == param.toUpperCase())?.color,
        borderWidth: 3,
        borderDash: [5, 5],
