...
const RunTest = () => {
	...
	const procedure = settings.account.testprocedures.find((procedure) => procedure.id === selectedMethod);
	...
	useEffect(() => {
		const checkFile = setInterval(() => {
			...
			if (resultPresent) {
				...
				setWaitingForResults(true);
				console.log("result for test " + testid + " found, waiting for writing process to finish...");
				window.api.logEvents(`Result for test ${testid} found, waiting for writing process to finish...`, "logInfos.txt");
				setTimeout(async () => {
				...
				window.api.logEvents(`Result for test ${testid} not present (yet)`, "logInfos.txt");
