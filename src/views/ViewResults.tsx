...
import { Table, Block, Checkmark, ButtonArea, Button, RackVisualRows, ChangeResults, AlteredResult, ResultsFooter } from "../components";
...
const ViewResults: React.FC = () => {
	...
	const { testid, barcodes, settings, testDone, setErrors, setBarcodes, viewResults, setViewResults, setSelectedMethod } = useData();
	...
	const getBarcode = (id: number) => {
		barcodes.map((barcode) => {
			if (barcode.id === id) {
				...
				window.api.logEvents(JSON.stringify(barcode), "logInfos");
			}
		});
	};

	const displayResults = async () => {
		window.api.logEvents(`displayResults: ${testid}`, "logInfos");
		//reset errors
		...
		try {
			...
			testmethod = settings.account.testprocedures.find((procedure: ITestProcedure) => procedure.id === testConfig.testmethod) as ITestProcedure;
			...
			<>
				<div className={`ViewResults `}>
					<div className={`ViewResultsHeader`}>
						<h3 style={{ textAlign: "start" }}>{viewResults == "graph" ? t("viewResults.instructions") : t("viewResults.instructions2")}</h3>
					...
					{viewResults === "graph" && (
						<>
							<RackVisualRows markActive={setActive} active={active} testmethod={testmethod as ITestProcedure} showResults={true} />
							<ResultsFooter activeBarcode={activeBarcode as IBarcode} testmethod={testmethod as ITestProcedure} />
						</>
					)}
					{/* TABLE VIEW */}
					{viewResults === "table" && (
									...
									...barcodes.map((barcode) => {
										return [
											...
											...((testmethod &&
												testmethod.parameters.map((parameter) => {
													...
													return parameter.showFL
														? barcode.parameters?.[target]
															? parseInt(barcode.parameters?.[target]?.finalCycle)
															: parseInt(barcode.parameters?.[target.toLowerCase()]?.finalCycle)
														: false;
												})) ||
												[]),
											...
											testmethod && testmethod.showCurves && (
												<div key={barcode.id.toString()} className="viewCurve" onClick={() => navigate(`/viewCurves/${barcode.id}`)}>
													<VscGraphLine />
												</div>
											),
										];
