...
const Bootup = () => {
  ...
  const getPairingCode = useCallback(async () => {
    ...
    if (settings.device.hardwareId === null) {
      ...
      window.api.logEvents(
        "Couldnt read MAC Adress from Settings.",
        "logInfos"
      );
      setErrors(
        errors
      ...
      try {
        ...
        window.api.logEvents(
          `getPairingCodeApi: ${JSON.stringify(response)}`,
          "logInfos"
        );
        window.api.logEvents(
          `PairingCodeApi: ${JSON.stringify(response?.data?.code)}`,
          "logInfos"
        );
      } catch (err: any) {
        console.log(err);
  ...
  const authenticateToken = useCallback(async () => {
    ...
    try {
      ...
      window.api.logEvents(
        ` authenticateToken response :${JSON.stringify(response)}`,
        "logInfos"
      );
      setLoading(false);
      if (response.ok) {
      ...
      if (
        (response?.problem && response?.problem == "NETWORK_ERROR") ||
        response?.problem == "CONNECTION_ERROR"
      ) {
        window.api.logEvents(
  ...
  useEffect(() => {
    if (settings.account.initialized) {
      ...
      window.api.logEvents(
        "initialized, attempting authentication",
        "logInfos"
      );
    } else {
      getPairingCode();
