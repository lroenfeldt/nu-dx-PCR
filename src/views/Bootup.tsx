...
const Bootup = () => {
	...
	const appendError = (type, messageKey, additional = {}) => {
		setErrors(errors.filter((error) => error.type !== type).concat({ type, message: t(messageKey, additional) }));
	};
	...
	const getPairingCode = useCallback(async () => {
		...
		try {
			const response = await getPairingCodeApi.request(settings.device.hardwareId, {
	...
	const authenticateToken = useCallback(async () => {
		...
		try {
			const response = await checkTokenApi.request(settings.device.hardwareId, { token });
			setLoading(false);
	...
	const handleNetworkError = (remainingDays) => {
		if (settings.account.allowOffline) {
			if (remainingDays > 0) {
				appendError("offline", "errors.deviceAuthenticationFailedUseOfflineMode", { days: remainingDays });
			} else {
