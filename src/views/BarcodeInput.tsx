...
const BarcodeInput = () => {
	...
	const checkBarcodeUrl = settings.account.customCheckBarcodesEndpoint ? settings.account.customCheckBarcodesEndpoint : urls.checkBarcodeUrl;
	...
	const nextWell = (next: boolean) => {
		let nextWell = active;
		const blockedBarcodes = barcodes.filter((barcode) => barcode.blocked === true).map((blockedBarcode) => blockedBarcode.id);
		setDisabled(false);
		...
		if (active <= wellCount && wellCount == 96) {
			for (let j = -13; j <= wellCount; j++) {
				if (next === true) {
					...
					if (j === nextWell && j >= 85 && blockedBarcodes.includes(nextWell - 83)) return markActive(nextWell - 83 + 12);
					...
					if (j === nextWell && blockedBarcodes.includes(nextWell - 12) && blockedBarcodes.includes(nextWell - 24)) return markActive(96);
					if (j === nextWell && j > 12 && j <= 24 && nextWell - 12 === 1 && blockedBarcodes.includes(nextWell - 12)) return markActive(96);
					if (j === nextWell && blockedBarcodes.includes(nextWell + 12)) return markActive(nextWell + 24);
					if (j === nextWell && j > 12 && j <= 24 && blockedBarcodes.includes(nextWell - 12)) return markActive(j - 12 + 83);
		...
		if (active === wellCount) {
			checkBarcode(getBarcode(active));
		}
	};
	...
	const retest = async (barcode: IBarcode) => {
			...
			try {
				...
				if (err.response) {
					...
					window.api.logEvents(
						`status: ${err.response.status} headers: ${err.response.headers} data: ${JSON.stringify(err.response.data)}`,
						"logErrors"
					);
					dbError = true;
					newErrors.push({
					...
					window.api.logEvents(`request: ${JSON.stringify(err.request)}`, "logErrors");
					dbError = true;
					newErrors.push({
						type: "dbCon",
		...
		setBarcodes((prevBarcodes) => prevBarcodes.map((prevBarcode) => (newBarcode.id === prevBarcode.id ? newBarcode : prevBarcode)));
	};
	...
	const checkBarcode = async (newBarcode: IBarcode) => {
		...
		if (!settings.account.verifyBarcodes) {
			setBarcodes((prevBarcodes) =>
				prevBarcodes.map((barcode) => (barcode.id === newBarcode.id ? { ...barcode, checking: false, valid: true } : barcode))
			);
			return;
		}
		...
		barcodes.forEach((barcode) => {
			if (barcode.error && barcode.error.includes(newBarcode.posName)) {
				const errBarcode = barcodes.find((barcode) => barcode.error && barcode.error.includes(newBarcode.posName)) as IBarcode;
				setBarcodes((prevBarcodes) =>
					prevBarcodes.map((barcode) => (barcode.id === errBarcode.id ? { ...barcode, error: "", valid: false } : barcode))
				);
			}
			if (barcode.error && barcode.error.includes(newBarcode.posName)) {
				const errBarcode = barcodes.find((barcode) => barcode.error && barcode.error.includes(newBarcode.posName)) as IBarcode;
		...
		//Check against db
		if (isValid && newBarcode.value.indexOf("-R") === -1 && settings.account.checkBarcodesDB && settings.account.verifyBarcodes) {
			if (offlineMode) {
	...
	return (
		<>
			<div className={`BarcodeInput ${isNinetySix ? " ninetySix" : ""}`}>
				...
				<RackVisualRows active={active} markActive={markActive} showResults={false} testmethod={testmethod} />
				...
				<div className={`inputContainer ${isNinetySix ? " ninetySix" : ""}`}>
					...
					<form action="" onSubmit={(e) => handleBarcodeSubmit(e)}>
						{isNinetySix && (
							<Block column gap={10} shadow radius={10} padding={12} marginBottom={keyboardActive ? -180 : ""}>
