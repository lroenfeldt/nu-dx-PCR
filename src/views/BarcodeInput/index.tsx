import { Block, Errors, Keyboard, Controller, WellVisual, RackVisualRows, ActivateKeyboard, Button, Text } from "../../components";
...
const BarcodeInput = () => {
	const { demo, reset, errors, reboot, loading, barcodes, settings, toggleLid, setErrors, setLoading, offlineMode, setBarcodes, isNinetySix } = useData();
	...
	const { checkBarcodeValidity, checkControlPlaceholders } = useBarcodeProperties();
		...
		async (id: number) => {
			...
			if (!settings.account.verifyBarcodes) {
				...
				setBarcodes((prevBarcodes) =>
					prevBarcodes.map((prevBarcode) => (newBarcode.id === prevBarcode.id ? { ...prevBarcode, checking: false, valid: true } : prevBarcode))
				);
				return;
			}
			...
			if (checkedControl) {
				setBarcodes((prevBarcodes) =>
					prevBarcodes.map((prevBarcode) => (newBarcode.id === prevBarcode.id ? { ...prevBarcode, ...checkedControl } : prevBarcode))
				);
				return;
			}
			...
			if (isValid && newBarcode.value.indexOf("-R") === -1 && settings.account.checkBarcodesDB) {
				...
				setBarcodes((prevBarcodes) =>
					prevBarcodes.map((prevBarcode) =>
						newBarcode.id === prevBarcode.id ? { ...prevBarcode, checking: false, valid: isValid, error: validationErr as string } : prevBarcode
					)
				);
			}
		},
	...
	const retest = async (barcode: IBarcode) => {
		...
		setBarcodes((prevBarcodes) =>
			prevBarcodes.map((prevBarcode) =>
				newBarcode.id === prevBarcode.id
					? { ...prevBarcode, value: newBarcodeValue, valid: true, checking: false, error: t("errors.testSample"), askRetest: false }
					: prevBarcode
			)
		);
	};
		...
		setBarcodes((prevBarcodes) =>
			prevBarcodes.map((barcode) => {
				if (barcode.id === active) {
					return {
						...barcode,
						askRetest: settings.account.allowRetest && settings.account.allowRetest,
					};
				} else {
					return barcode;
				}
	...
	return (
		<Block flex center height={"100%"} width="100%">
			<Block flex center column gap={20}>
				...
				/>
				<ActionsSection active={active} barcodesValid={barcodesValid} getBarcode={getBarcode} />
				<BarcodeKeyboard
					active={active}
					nextWell={nextWell}
