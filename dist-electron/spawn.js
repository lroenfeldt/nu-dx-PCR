...
const fileName = `logs-${hooks().format("DD-MM-YYYY")}-${hooks().locale("de").format("LT")}.txt`;
const filePath = path.resolve(require$$0.app.getPath("userData"), "logs", fileName);
path.resolve(require$$0.app.getPath("userData"), "logs");
const { v4: uuid } = require("uuid");
const logger = (message) => {
  const dateTime = `${hooks().format("DD-MM-YYYY")}-${hooks().locale("de").format("LT")}`;
...
const spawn = async (cmd, args, options) => {
  ...
  return new Promise((resolve, reject) => {
    const child = options ? child_process.spawn(cmd, args, options) : child_process.spawn(cmd, args);
    let errorData = "";
    if (!child.stdout || !child.stderr) {
      reject(
        new Error(
          `Failed to create child process for cmd: ${cmd} with args: ${args.join(
            " "
          )}. Either stdout or stderr was null.`
        )
      );
      return;
    }
    child.stdout.on("data", (data) => {
      logger(`success ${cmd} ${args.join(" ")}: ${data}`);
    });
